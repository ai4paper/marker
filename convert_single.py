...
if __name__ == "__main__":
    ...
    parser.add_argument("--parallel_factor", type=int, default=1,
                        help="How much to multiply default parallel OCR workers and model batch sizes by.")
    args = parser.parse_args()

    fname = args.filename
    model_lst = load_all_models()
    full_text, out_meta = convert_single_pdf(fname, model_lst, max_pages=args.max_pages,
    ...
    with open(out_meta_filename, "w+", encoding='utf-8') as f:
        f.write(json.dumps(out_meta, ensure_ascii=False, indent=4))
